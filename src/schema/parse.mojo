...
def _parse_type(v: ReadValue, mut doc: SchemaDoc, name: String) raises DecodeError -> Int:
    _reject_unknown(v)
    if _has(v, "x-yaml-binary"):
        var flag = v.get("x-yaml-binary")
...
def _parse_object(v: ReadValue, mut doc: SchemaDoc, name: String) raises DecodeError -> Int:
    ...
    if _has(v, "x-yaml-style"):
        var enc = v.get("x-yaml-style").as_str()
        if enc == "array":
    ...
    if _has(v, "properties"):
        ...
        while i < props.count():
            ...
            var ik = Int64(i)
            var has_ik = False
            if _has(p[1], "x-yaml-plain"):
                ik = p[1].get("x-yaml-plain").as_int()
                has_ik = True
